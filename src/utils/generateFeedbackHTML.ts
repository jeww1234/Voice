export function generateFeedbackParts(result: any) {
  ...
  if (result.finalScore === 100) {
    summary += `<div class="feedback-perfect">🌟 <strong>완벽합니다!</strong></div>`;
    summary += `<p>모든 발음과 입모양이 정확합니다. 정말 훌륭해요!</p>`;
  } else if (result.finalScore >= 90) {
    summary += `<div class="feedback-excellent">🎉 <strong>훌륭합니다!</strong></div>`;
    summary += `<p>거의 완벽에 가까워요. 아주 잘하셨어요!</p>`;
  } else if (result.finalScore >= 70) {
    summary += `<div class="feedback-good">👍 <strong>잘하셨습니다!</strong></div>`;
    summary += `<p>대부분 정확하지만 몇 가지 개선할 부분이 있습니다.</p>`;
  } else if (result.finalScore >= 50) {
    summary += `<div class="feedback-fair">💪 <strong>좋은 시도입니다!</strong></div>`;
    summary += `<p>더 연습이 필요합니다. 아래 피드백을 참고하세요.</p>`;
  } else {
    summary += `<div class="feedback-poor">📚 <strong>더 연습해봅시다!</strong></div>`;
  ...
  if (errors.length > 0) {
    ...
    improvementTips += `<div class="improvement-tips" style="background: #d4edda; border-left-color: #28a745;">
      <h4>✅ 완벽합니다!</h4>
      <p>모든 발음이 정확합니다. 계속 연습하세요!</p>
    </div>`;
  }
